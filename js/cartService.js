...
function agregarAlCarrito(producto){
  ...
  let memoria = JSON.parse(localStorage.getItem("catalogo"));
  ...
  if(!memoria || memoria.length === 0) {
    ...
    localStorage.setItem("catalogo",JSON.stringify(nuevaMemoria));
    actualizarNumeroCarrito();
...
function actualizarNumeroCarrito(){
  let cuenta = 0;
  const memoria = JSON.parse(localStorage.getItem("catalogo"));
  if(memoria && memoria.length > 0){
